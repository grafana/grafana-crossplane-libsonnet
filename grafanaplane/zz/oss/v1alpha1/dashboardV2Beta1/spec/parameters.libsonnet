  ...
  forProvider+:
    {
      ...
      options+:
        {
          '#withAllowUiUpdates': { 'function': { args: [{ default: true, enums: null, name: 'value', type: ['boolean'] }], help: '(Boolean) Set to true to allow editing the resource from the Grafana UI.\nSet to true to allow editing the resource from the Grafana UI.' } },
          withAllowUiUpdates(value=true): {
            spec+: {
              parameters+: {
                forProvider+: {
                  options+: {
                    allowUiUpdates: value,
                  },
                },
              },
            },
      ...
      spec+:
        {
          '#withJson': { 'function': { args: [{ default: null, enums: null, name: 'value', type: ['string'] }], help: '(String) The JSON representation of the dashboard v2beta1 spec. Must be the spec object only — not the full Kubernetes envelope. Use: json = jsonencode(jsondecode(file("dashboard.json")).spec)\nThe JSON representation of the dashboard v2beta1 spec. Must be the spec object only — not the full Kubernetes envelope. Use: json = jsonencode(jsondecode(file("dashboard.json")).spec)' } },
          withJson(value): {
            spec+: {
              parameters+: {
                forProvider+: {
