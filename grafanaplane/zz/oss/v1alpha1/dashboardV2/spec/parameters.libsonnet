  ...
  forProvider+:
    {
      ...
      spec+:
        {
          '#withJson': { 'function': { args: [{ default: null, enums: null, name: 'value', type: ['string'] }], help: '(String) The JSON representation of the dashboard v2 spec. Must be the spec object only — not the full Kubernetes envelope. Use: json = jsonencode(jsondecode(file("dashboard.json")).spec)\nThe JSON representation of the dashboard v2 spec. Must be the spec object only — not the full Kubernetes envelope. Use: json = jsonencode(jsondecode(file("dashboard.json")).spec)' } },
          withJson(value): {
            spec+: {
              parameters+: {
                forProvider+: {
